...
def init_app():
    ...
    if use_gpu:
        model = AutoModel.from_pretrained(settings.llm_model, trust_remote_code=True).half().cuda()
    else:
        model = AutoModel.from_pretrained(settings.llm_model, trust_remote_code=True).half().cpu()
    model.eval()
